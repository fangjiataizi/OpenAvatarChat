...
# AI在线教学平台启动脚本 - 存储层版本
# 使用PostgreSQL + Redis + MinIO三层存储架构
...
if [ "$ACTION" = "start" ]; then
    CONFIG_FILE=${2:-"config/teaching_with_storage.yaml"}
    HOST=${3:-"0.0.0.0"}
    PORT=${4:-"8443"}
else
    # restart命令
...
echo "   Frontend: src/frontend/teaching_frontend.py"
echo "   Backend:  src/backend/teaching_backend_with_storage.py"  
echo "   API:      src/api/teaching_api.py"
echo "   Main:     src/teaching_demo.py"
echo ""
...
if ps -p $PLATFORM_PID > /dev/null 2>&1; then
    ...
    echo ""
    echo "🎓 存储功能说明:"
    echo "   - 用户数据持久化存储在PostgreSQL"
    echo "   - AI处理结果缓存在Redis中"
    echo "   - 学习会话和进度自动保存"
    echo "   - 支持用户管理和学习分析"
    echo ""
    echo "📖 管理命令:"
